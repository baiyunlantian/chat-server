...
import { UserMap, FriendMessage } from '../friend/entity/friend.entity';
import { createWriteStream } from 'fs';
import { join } from 'path';
import { RCode } from 'src/common/constant/rcode';
import { GroupDto, GroupMessageDto, FriendDto, FriendMessageDto } from './dto/index.dto';
...
export class ChatGateway {
    ...
    if(isUser) {
      ...
      if (group && user) {
        if (!userGroup) {
          ...
          this.server.to(data.userId).emit('joinGroup', { code: RCode.FAIL, msg: '已在该群内，无需重复加入', data: '' });
          return
        }
  ...
  async getAllData(@ConnectedSocket() client: Socket,  @MessageBody() userId: string):Promise<any> {
    const isUser = await this.userRepository.findOne({userId});
    if(isUser) {
      let groupArr: Awaited<Group>[] = [];
      let friendArr: Awaited<User>[] = [];
      const userGather: {[key: string]: User} = {};
      let userArr: (User | Awaited<User>)[] = [];
    
      const groupMap: GroupMap[] = await this.groupUserRepository.find({userId});
      const friendMap: UserMap[] = await this.friendRepository.find({userId});
      ...
      this.server.to(userId).emit('chatData', {code:RCode.OK, msg: '获取聊天数据成功', data: {
        groupData: groupArr,
